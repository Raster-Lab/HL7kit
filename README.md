...
- **HL7 v2.x Parser Infrastructure**: Configurable parser with encoding detection, delimiter auto-detection, error recovery modes (strict, skip, best-effort), streaming support, segment validation, and diagnostic reporting. **Supports multiple character encodings** (ASCII, UTF-8, ISO-8859-1, Windows-1252) with automatic detection.
...
- **Z-Segment Support**: Complete support for custom Z-segments with segment definition registry, validation against custom schemas, and fluent builder API. Includes common Z-segment examples (ZPI, ZBE, ZOB).
- **Batch & File Processing**: Full support for batch (BHS/BTS) and file (FHS/FTS) structures per HL7 v2.x specifications. Includes parsing, validation, and serialization of batch and file containers.
- **Streaming API**: Memory-efficient streaming API for processing large HL7 files without loading entire contents into memory. Supports both message and batch streaming with async/await interface.
- **Compression Support**: Message and batch compression using Apple's Compression framework (LZFSE, LZ4, ZLIB, LZMA algorithms). Includes transparent compression/decompression with configurable compression levels.
...
- **High Test Coverage**: 889 unit tests with 97.5%+ passing (22 failures expected on Linux due to platform limitations)
