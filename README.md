...
- **CLI Tools**: Reusable command-line tool types for HL7 message processing including `MessageValidatorCLI` for message validation with detailed reports, `FormatConverterCLI` for converting between HL7 v2, JSON, XML, and pretty-print formats, `NetworkTestCLI` for MLLP connection testing and message exchange, `ConformanceCheckerCLI` for profile-based conformance checking with scoring, `BatchProcessorCLI` for multi-message batch operations, and `CLIOutputFormatter` for text/JSON report formatting. All types are public and Sendable. Includes 50 unit tests.
...
- **Platform Integrations**: Framework-agnostic protocols and data types for Apple platform integration including `HealthKitBridge` protocol with LOINC-to-HealthKit mapping (vital signs, lab results, medications), `CareKitBridge` protocol for care plan and task conversion, `ResearchKitBridge` protocol for survey/consent data exchange, `CloudSyncProvider` protocol for iCloud-based HL7 message sync with conflict resolution strategies, `HandoffProvider` protocol for NSUserActivity-based Handoff with typed activity payloads, and `ShortcutsProvider` protocol for Siri Shortcuts integration. All types are public, Sendable, Codable, and compile on Linux. Includes `PlatformIntegrationManager` actor for runtime bridge registration. 68 unit tests.
...
│   └── CLITools.swift         # CLI tool types (validator, converter, network test, conformance, batch)
├── Examples/          # Sample applications
│   ├── iOS/           # SwiftUI iOS demo app (message viewer, network testing, validation, performance)
│   ├── macOS/         # AppKit/SwiftUI macOS workstation (message processing, batch tools, dev tools, interface testing)
│   └── CLI/           # Command-line tool example (validation, conversion, conformance, batch processing)
